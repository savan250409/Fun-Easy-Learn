    ...
    <div class="page-header-custom">
        ...
        <div class="d-flex align-items-center">
            <div class="stats-badge me-3">
                <i class="mdi mdi-layers"></i> Total: <span id="totalCount">{{ $childCategories->total() }}</span> Child Categories
            </div>
        ...
        <div class="card-body">
            <div class="custom-search-wrapper">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <input type="text" id="searchInput" class="search-input-box" placeholder="Search child categories...">
                </div>
            </div>
            ...
            <div class="table-responsive">
                <table class="table-custom">
                    ...
                    <tbody id="tableBody">
                        @include('admin.child_categories._rows')
                    </tbody>
                </table>
            </div>
            ...
            <div class="d-flex justify-content-between align-items-center px-4 py-3 border-top"
                ...
                <div id="showingText">Showing {{ $childCategories->firstItem() ?? 0 }} to {{ $childCategories->lastItem() ?? 0 }} of {{ $childCategories->total() }} entries</div>
                <div id="paginationWrapper">{{ $childCategories->links('pagination::bootstrap-4') }}</div>
            </div>
        </div>
    </div>
@endsection
...
<script>
    ...
    function fetchData() {
        const params = new URLSearchParams({
            ...
            sub_category_id: currentFilter,
            per_page:        currentPerPage,
            page:            currentPage,
        });

        fetch('{{ route('child-categories.index') }}?' + params.toString(), {
            headers: {
